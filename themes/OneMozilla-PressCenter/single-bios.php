...
<?php $position = get_post_meta($nav->post->ID, 'bio_position', true); ?>
    <li <?php if ($wp_query->post->ID === $nav->post->ID) : echo 'class="current"'; endif; ?>>
...
?>
  <article class="hentry vcard bio" id="bio-<?php the_ID(); ?>">
    ...
    <div class="entry-content">
      ...
      <?php if ($blog_url) : ?>
        <p class="bio-extra"><?php _e('Website','mozpress'); ?>: <a href="<?php echo esc_attr($blog_url); ?>" rel="external" class="url"><?php echo esc_attr($blog_name); ?></a></p>
      <?php endif; ?>
      <?php if ($twitter) : ?>
